...
void Init_EPROM()
{
 ...
 _SYSINFO tmpsysinfo={0}; //={0,0};
...

	{
		...
		FLASH_Read((BYTE *) &SysInfo, (FLADDR) (EEP_SYS_TBLSTART),
				EEP_SYS_LENTH);
	...
	else
	{
		...
		SysInfo.ADJUST = 0;
		SysInfo.VA = (WORD) ((DWORD) DEFAULT_NOCOAL_VA * 1024
				/ STANDARD_REF_VOLT)|0x8000; //校准
		SysInfo.VB = (WORD) ((DWORD) DEFAULT_NOCOAL_VB * 1024
				/ STANDARD_REF_VOLT); //校准
		SysInfo.VC = (WORD) ((DWORD) DEFALT_130PPM_VC * 1024
				/ STANDARD_REF_VOLT); //130PPM
...
void main(void)
{
	...
	while (1)
	{
		...
		if (F_100ms)
		{
			extern void BatteryEnegyDetect();
