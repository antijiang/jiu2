...
CalALOTwo(WORD vol)
{
    ...
    if (VC > SysInfo.VC20)  //<20mg
    {
        ...
        pcal = 20 + (VC - SysInfo.VC20) * (20 - 50) / (SysInfo.VC20 - SysInfo.VC);
    if (pcal < 0)
        pcal = 0;
    if (pcal > 150)
        pcal = 150;
...
LABEL_CMP:
            if ((WORD)Dtempv > VB	 //标定130PPM 电压要低于 0点0.6 V以上，否则无效
               )
            {
                DisplayCont = DISPLAY_ERR2;
                StateSensor = AL_STATE_EXIT;
...
LABEL_TEST_ALO:
    ...
    case KEY_DISPLAY:
        ...
        {
            DisplayCont++;
            if (DisplayCont > DISPLAY_SYS_VCEND || DisplayCont < DISPLAY_SYS_RAN)
