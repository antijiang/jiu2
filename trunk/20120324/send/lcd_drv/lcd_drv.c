...
void DIsplaySYSV(WORD hexnum)
{
	...
	tmp = (WORD) ((DWORD) tmp * (STANDARD_REF_VOLT) / 1024); //显示mV
...
void Display_All()
{
	...
	case DISPLAY_SYS_VC:
		...
		break;
	case DISPLAY_SYS_VC20:
		DIsplaySYSV(SysInfo.VC20);
	...
	if (DisplayCont >= DISPLAY_SYS_RAN && DisplayCont <= DISPLAY_SYS_VCEND)
	{
		if (F_half_sec)
		{
